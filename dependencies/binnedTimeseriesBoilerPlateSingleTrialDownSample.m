% same as single trial but will downsample to 10hz
...
% 
timePad = timePad/downsampleamount;
total_time_ = obj.iv.total_time_/downsampleamount;
lampOff = round(obj.GLM.pos.lampOff/downsampleamount);
fLick = round(fLick/downsampleamount);
ts = ts(1:downsampleamount:end);
...
for ibin = 1:nbins
	...
	mask2 = ~ismember(teb, 1);
	lick_times_previous_trial{ibin}(mask2) = obj.GLM.flick_s_wrtc(teb(mask2)-1); 
	...
	% 
    obj.ts.BinnedData.CTA{ibin} = zeros(1,2*timePad + total_time_ + 1);
	...
	for n = 1:numel(trials_in_each_bin{ibin})
		c1 = lampOff(trials_in_each_bin{ibin}(n)) - timePad;
		c2 = lampOff(trials_in_each_bin{ibin}(n)) + total_time_ + timePad;
		if c1 < 0
...
obj.ts.Plot.CTA.xticks.s = [-timePad:timePad+total_time_]/(1000/downsampleamount)/samples_per_ms_xticks;
obj.ts.Plot.LTA.xticks.s = [-timePad:timePad]/(1000/downsampleamount)/samples_per_ms_xticks;
...
% obj.ts.Plot.CTA.xticks.s = obj.ts.Plot.CTA.xticks.s(1:downsampleamount:end);
% obj.ts.Plot.LTA.xticks.s = obj.ts.Plot.LTA.xticks.s(1:downsampleamount:end);