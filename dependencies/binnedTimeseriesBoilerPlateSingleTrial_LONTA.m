...
for ibin = 1:nbins
	...
	% 
    % obj.ts.BinnedData.CTA{ibin} = zeros(1,2*timePad + 1);
	...
	for n = 1:numel(trials_in_each_bin{ibin})
		% c1 = obj.GLM.pos.lampOn(trials_in_each_bin{ibin}(n)) - timePad;
		% c2 = obj.GLM.pos.lampOn(trials_in_each_bin{ibin}(n)) + timePad;
		% if c1 < 0
		...
		l1 = obj.GLM.pos.lampOn(trials_in_each_bin{ibin}(n)) - timePad;
		l2 = obj.GLM.pos.lampOn(trials_in_each_bin{ibin}(n)) + timePad;
		% l1 = fLick(trials_in_each_bin{ibin}(n)) - timePad;
		% l2 = fLick(trials_in_each_bin{ibin}(n)) + timePad;
        if strcmpi(obj.iv.signaltype_, 'none')&&l1 <1
            warning('edge cut off?')
            l1=1;
        end
        % if strcmpi(obj.iv.signaltype_, 'none')&&c1 <1
        %     warning('edge cut off?')
        %     c1=1;
        % end
        if strcmpi(obj.iv.signaltype_, 'none')&&l2 >numel(ts)
            warning('edge cut off?')
            l2=numel(ts);
        end
        % if strcmpi(obj.iv.signaltype_, 'none')&&c2 >numel(ts)
        %     warning('edge cut off?')
        %     c2=numel(ts);
        % end
		if l1 < 0
        ...
        elseif l2 > numel(ts)
			...
			nL(isnan(nxt)) = nL(isnan(nxt)) - 1;
			nL(nL==0) = nan;
            if strcmpi(obj.iv.signaltype_, 'none') && sum(size(nxt) == size(nL)) ~=2
                warning('RBF IF IT''S NOT ARCHER DAY 38...edge cut off? we are excluding this trial YOU SHOULD DEBUG THIS IF ITS NOT FEB 2024!')
            else
                nxt = nxt./nL; % keeps nan in place
			    nxt = nxt; % keeps nan in place
                obj.ts.BinnedData.LTA{ibin} = nansum([obj.ts.BinnedData.LTA{ibin} .* ((nL-1)./nL); nxt]); % ignores the nans
            end
...
% 	Append the legend
% 
obj.ts.BinParams.Legend_s.CLTA = [arrayfun(@(x) [num2str(x) 's | n=1'],round(obj.GLM.flick_s_wrtc,3), 'uniformoutput', 0)]';
% 
% 	Get Bin Time Centers and Ranges
% 
obj.ts.BinParams.s(ibin).CLTA_Min = obj.GLM.flick_s_wrtc;
obj.ts.BinParams.s(ibin).CLTA_Max = obj.GLM.flick_s_wrtc;
obj.ts.BinParams.s(ibin).CLTA_Center = obj.GLM.flick_s_wrtc;

obj.ts.BinParams.binEdges_CLTA = obj.GLM.flick_s_wrtc;
...
obj.ts.warning = 'LONTA processed. LTA field is aligned to LAMP ON. There is no CTA here.';


% we also need to remove any lampoff where there was an early lick...that's not really the point -- 
lampOff_times_by_trial = obj.GLM.lampOn_s - obj.GLM.cue_s;

fcc = obj.GLM.flick_s_wrtc;
fcc(isnan(fcc)) = 100;
obj.ts.ITISurpriseTrials = find(lampOff_times_by_trial < fcc)';
obj.ts.noSurpriseTrials = find(lampOff_times_by_trial > obj.GLM.flick_s_wrtc)';
